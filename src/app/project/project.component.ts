...
export class ProjectComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  private loadAllNotifications(): void {
    ...
    this.wsService.getUserNotifications(receiverId).subscribe({
      next: (notifs: any[]) => {
        this.notifications = notifs.map((n: Notification) => ({
          ...n,
  ...
  markAllAsRead(): void {
    ...
    this.wsService.markAllAsRead(receiverId).subscribe({
      ...
      error: (err: any) => this.showError('Failed to update notifications')
    });
  }
  ...
  clearAll(): void {
    ...
    this.wsService.clearAllNotifications(receiverId).subscribe({
      ...
      error: (err: any) => this.showError('Failed to clear notifications')
    });
  }
